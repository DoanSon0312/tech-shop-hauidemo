...
public class VNPayService {
    ...
    // Sửa phương thức createOrder để nhận thêm tham số orderInfo
    public String createOrder(HttpServletRequest request, int amount, String urlReturn, String orderInfo) {
        String vnp_Version = "2.1.0";
        String vnp_Command = "pay";
        String vnp_TxnRef = VNPayConfig.getRandomNumber(8);
        String vnp_IpAddr = VNPayConfig.getIpAddress(request);
        ...
        vnp_Params.put("vnp_Command", vnp_Command);
        vnp_Params.put("vnp_TmnCode", vnp_TmnCode);
        String totalAmount = String.valueOf(amount * 100);
        vnp_Params.put("vnp_Amount", totalAmount);
        vnp_Params.put("vnp_CurrCode", "VND");

        vnp_Params.put("vnp_TxnRef", vnp_TxnRef);
        vnp_Params.put("vnp_OrderInfo", orderInfo); // Sử dụng orderInfo từ tham số
        vnp_Params.put("vnp_OrderType", orderType);

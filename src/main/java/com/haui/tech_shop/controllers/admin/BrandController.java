package com.haui.tech_shop.controllers.admin;

import com.haui.tech_shop.dtos.requests.BrandRequest;
import com.haui.tech_shop.entities.Brand;
import com.haui.tech_shop.services.interfaces.IBrandService;
...
public class BrandController {
    ...
    @GetMapping("")
    public String listBrands(@RequestParam(required = false) Long brandId,
                             Model model) {

        List<Brand> brands;

        if (brandId != null) {
            // tự viết hàm trong service để trả về list theo id
            brands = brandService.findByIdAsList(brandId);
        } else {
            brands = brandService.findAll();
        }

        model.addAttribute("brands", brands); // danh sách brand để hiển thị bảng
        model.addAttribute("allBrands", brandService.findAll()); // đổ vào combobox
        model.addAttribute("selectedBrandId", brandId); // giữ option đã chọn
